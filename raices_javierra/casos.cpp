...
void  primerParcial()
{
	...
	string str_func = "5 * (2^2 * acos_alt((2 - x) / 2) - (2 - x) * sqrt(2 * 2 * x - x^2)) - 8.5";

	// Continuación de la función caso_especifico
	double xInf = 0.0f; // Como mínimo, h puede ser 0
	double xSup = 2.0f; // Como máximo, h puede ser igual a r
	double tol = 1e-7f; // La tolerancia especificada en el PDF
	int n = 100;		// Un número máximo de iteraciones

	// Crear una instancia del método
	cout<<"Solucion por biseccion"<<endl;
	cout<<"Intervalo: ["<<xInf<<" ... "<<xSup<<"]"<<endl;
	biseccion b(str_func);
	// Encontrar la raíz en el intervalo [xInf, xSup]
	solucion sol = b.encontrar(xInf, xSup, tol, n);
	...
	// Crear una instancia del método de regla falsa
	double xi = 0.0f; // Extremo inferior del intervalo
	double xs = 2.0f; // Extremo superior del intervalo, ya que h <= r

	// Crear una instancia de regla_falsa
	cout << "\nSolucion por regla falsa" << endl;
	cout << "Intervalo: [" << xi << " ... " << xs << "]" << endl;
	regla_falsa rf(str_func);
	// Encontrar la raíz usando el método de regla falsa
	solucion sol_rf = rf.encontrar(xi, xs, tol, n);
	// Imprimir la solución
	imprimir_solucion(sol_rf);
	...
	cout << "\nSolucion por el metodo de secante:" << endl;
	cout << "Intervalo: [" << x0 << "..." << x1<<"]" << endl;
	// Crear una instancia del metodo
	secante sec(str_func);
	// Encontrar la raiz en el intervalo dado
